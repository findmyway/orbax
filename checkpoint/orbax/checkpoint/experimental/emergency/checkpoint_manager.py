...
def _common_values_per_slice(
    ...
    *,
    ...
    logging.info(
        "global_mesh: %s, total_num_slices: %d, num_processes_per_slice: %d",
        global_mesh,
        total_num_slices,
        num_processes_per_slice,
    )
    ...
    logging.info("Intermediate per_slice_values: %s", per_process_values)

    for slice_id, values in per_slice_values.items():
        counter = collections.Counter(values)
        common_values = [k for k in counter if counter[k] == num_processes_per_slice]
...
def _process_local_to_global(
    ...
    *,
    ...
    broadcast_dir_key = (
        multihost._unique_barrier_key(broadcast_dir_key) + "/"
    ...
    barrier_key = multihost._unique_barrier_key(
        barrier_key
    )  # pylint: disable=protected-access
    logging.vlog(
        1,
...
def _global_max(
    ...
    for d in jax.local_devices():
        sdas.append(jax.device_put(np.asarray(values).reshape((1, 1, len(values))), d))
...
class _MultisliceCheckpointManager(
    ...
    def save(
        ...
        *,
        ...
        multihost.sync_global_processes(
            ...
            record_event_name=("/jax/orbax/write/checkpoint_start_sync_duration_secs"),
        )
    ...
    def _restore_from_local(
        ...
        if is_restoring_slice:
            ...
            single_slice_pytree = local_state_handler.restore(
                restore_directory / _STATE_ITEM_NAME,
                args=dataclasses.replace(args, restore_args=single_slice_restore_args),
            )
    ...
    def _restore_from_persistent(
        ...
        args = args_lib.PyTreeRestore(
            item=self._abstract_state,
            restore_args=checkpoint_utils.construct_restore_args(self._abstract_state),
        )
