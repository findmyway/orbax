...
async def read_and_create_array(
    ...
    *,
    ...
    for d, idx in _get_device_to_index_map(global_shape, sharding).items():
        if (
            d in sharding._addressable_device_assignment
...
async def async_deserialize(
    ...
    *,
    ...
    context = context or ts_utils.get_ts_context(use_ocdbt=False)
    sharding = (
        user_sharding.sharding if isinstance(user_sharding, Layout) else user_sharding
    )
    ...
    dll = (
        user_sharding.device_local_layout if isinstance(user_sharding, Layout) else None
    )
    t = await ts.open(
        tensorstore_spec,
        open=True,
