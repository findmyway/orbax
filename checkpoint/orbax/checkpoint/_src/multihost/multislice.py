...
def _process_in_device_slice(process_index: int, device_slice: np.ndarray) -> bool:
...
def tree_memory_per_device(tree: Tuple[PyTree, ...]) -> int:
    ...
    leaf_memory_per_device = jax.tree_util.tree_map(get_leaf_memory_per_device, tree)
    return jax.tree.reduce(lambda x, y: x + y, leaf_memory_per_device)


def get_available_memory(in_tree: Tuple[PyTree, ...], scaling_factor: float) -> int:
